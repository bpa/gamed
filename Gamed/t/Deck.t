...
is($d.elems, 0, "No cards after drawing the last one");

$d.add(@hearts);
is($d.elems, 13, "Can add an array of cards");

my @hand = $d.draw(13);
is_deeply(@hand, @hearts, "Drawing all cards returns them in order");
is($d.elems, 0, "After drawing all cards, you have none");
is(@hand.elems, 13, "Return is an array");

$d.add(@hearts);
$d.shuffle;
is($d.elems, 13, "Still have all the cards after shuffling");
@hand = $d.draw(13);
...
is($d.elems, 0, "Drawing whatever leaves the deck empty");
is_deeply(@hand, @hearts, "Drawing all cards via whatever returns them in order");

$d.add(@hearts);
@hand = $d.pick;
is($d.elems, 12, "Picking a card pulls one at random");
is(@hand.elems, 1, "The random card is returned");

@hand.push($d.pick(2));
is($d.elems, 10, "Picking 2 cards removes them from the deck");
is(@hand.elems, 3, "Picking 2 cards returns 2 cards");

@hand.push($d.pick(*));
is($d.elems, 0, "Picking all cards empties the deck");
...
is($d.elems, 10, "Drawing some cards should leave a smaller deck, but not empty");
